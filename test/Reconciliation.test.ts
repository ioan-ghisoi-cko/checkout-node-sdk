...
describe("Reconciliation", async () => {
	...
	it("should get payments based on filters", async () => {
		nock("https://api.checkout.com")
			.get("/reporting/payments?reference=ORD-5023-4E89")
			.reply(200, {
				"count": 1,
				"data": [
					{
		...
		const outcome = await rec.getPayments({
			reference: "ORD-5023-4E89"
		});
		expect(outcome.data[0].reference).to.equal('ORD-5023-4E89');
	});
