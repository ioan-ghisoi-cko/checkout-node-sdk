...
export const validatePayment = request => {
    ...
    if (
        ...
        !(request.payment_type.toLowerCase() in PAYMENT_TYPES)
    ) {
    ...
    if (
        request.source &&
        request.source.type &&
        !(typeof request.source.type === "string")
    ) {
...
export const setSourceOrDestinationType = request => {
    if (request.source) {
        if (request.source.type) {
            return request;
        } else if ("number" in request.source) {
            ...
            "id" in request.source &&
            request.source.id.startsWith("src_")
        ) {
            request.source.type = "id";
        } else if (
            "token" in request.source &&
            !("cryptogram" in request.source)
        ) {
        ...
        if (request.destination.type) {
            return request;
        } else if ("number" in request.destination) {
...
export const setTokenType = request => {
    if (request.type) {
        return request;
    } else if ("number" in request) {
        request.type = "card";
    } else if ("token_data" in request && "header" in request.token_data) {
        request.type = "applepay";
    } else if (
        "token_data" in request &&
        "signedMessage" in request.token_data
    ) {
...
export const setSourceType = request => {
    if (request.type) {
        ...
        "source_data" in request &&
        "account_type" in request.source_data
    ) {
        request.type = "ach";
    } else if (
        "source_data" in request &&
        "account_iban" in request.source_data
    ) {
        request.type = "sepa";
    } else {
        return request;
